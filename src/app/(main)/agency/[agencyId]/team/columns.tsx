            ...
            <div className="flex flex-col items-start">
              <div className="flex flex-col gap-2">
                <Badge className="bg-slate-600 whitespace-nowrap">
                  Agência - {row?.original?.Agency?.name}
                </Badge>
              </div>
            </div>
          );
            ...
            <div className="flex flex-col gap-2">
              {ownedAccounts?.length ? (
                ownedAccounts.map((account) => (
                  ...
                  >
                    Subconta - {account.SubAccount.name}
                  </Badge>
                ))
              ) : (
                <div className="text-muted-foreground">Sem acesso ainda</div>
              )}
            </div>
          </div>
        );
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            ...
            onClick={() => {
              setOpen(
                <CustomModal
                  subheading="Você pode alterar as permissões somente quando o usuário tiver uma subconta própria"
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-left">
            Essa ação não pode ser desfeita. Isso vai deletar o usuário
            permanentemente e todos os seus dados relacionados.
          </AlertDialogDescription>
        </AlertDialogHeader>
          ...
          <AlertDialogAction
            ...
            onClick={async () => {
              ...
              toast({
                ...
                description:
                  "Esse usuário foi deletado da agência e não possui mais acesso ",
              });
              setLoading(false);
              router.refresh();
            }}
