...
export const saveActivityLogsNotification = async ({
  ...
  if (!foundAgencyId) {
    if (!subAccountId) {
      throw new Error(
        "You need to provide at least an agency ID or subaccounnt ID"
      );
    }
...
export const upsertSubAccount = async (subAccount: SubAccount) => {
  ...
  if (!agencyOwner) return console.log("Agency owner could not be found");
...
export const upsertTag = async (
  ...
  const response = await db.tag.upsert({
    where: { id: tag.id || v4(), subAccountId },
    update: tag,
    create: { ...tag, subAccountId },
  })
