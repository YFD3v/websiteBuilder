...
const TagCreator = ({ getSelectedTags, subAccountId, defaultTags }: Props) => {
  ...
  const handleAddTag = async () => {
    if (!value) {
      toast({
        variant: 'destructive',
        title: 'Tags precisam ter um nome',
      })
      return
    }
    if (!selectedColor) {
      toast({
        variant: 'destructive',
        title: 'Por favor selecione uma cor',
      })
      return
    }
    const tagData: Tag = {
    ...
    try {
      ...
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Atualizou a tag | ${response?.name}`,
        subAccountId: subAccountId,
      })
    } catch (error) {
      toast({
        variant: 'destructive',
        title: 'Não foi possível criar tag',
      })
    }
  }
  ...
  const handleDeleteTag = async (tagId: string) => {
    ...
    try {
      ...
      toast({
        title: 'Tag excluída',
        description: 'A tag foi excluida da sua subconta.',
      })

      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Exclui a tag | ${response?.name}`,
        subAccountId: subAccountId,
      })
              ...
              >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    sua tag e remova-a de nossos servidores.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
